...
function normalizeCdCode(cd) {
	if (!cd) return "";
	const str = String(cd).toUpperCase().trim().replace(/[-\s]/g, '');
	const m = str.match(/^([A-Z]{2})(AL|0*(\d+))$/);
	if (m) {
		const state = m[1];
		const num = m[2] === "AL" ? "AL" : String(parseInt(m[3], 10)).padStart(2, '0');
		return `${state}-${num}`;
	}
	return str;
}
...
function matchCdCodes(cd1, cd2) {
	...
	return normalizeCdCode(cd1) === normalizeCdCode(cd2);
}
...
function getFeatureCdCode(f) {
    ...
    if (p.name && /^[A-Z]{2}-(AL|\d{1,2})$/i.test(p.name)) return normalizeCdCode(p.name);
    if (p.cd_code) return normalizeCdCode(p.cd_code);
    if (p.CD_CODE) return normalizeCdCode(p.CD_CODE);
    if (p.cd && /^[A-Z]{2}-(AL|\d{1,2})$/i.test(p.cd)) return normalizeCdCode(p.cd);
    ...
    if (stateVal && cdVal !== undefined) {
        ...
        if (STATE_PO_TO_FIPS[stPo]) {
            const cdStr = String(cdVal).toUpperCase() === "AL" ? "AL" : String(parseInt(cdVal, 10)).padStart(2, '0');
            return `${stPo}-${cdStr}`;
        }
    }
    ...
    if (p.GEOID) {
        ...
        if (geoid.length >= 4) {
            ...
            if (stPo) {
                const numVal = parseInt(cdNum, 10);
                const cdStr = (isNaN(numVal) || numVal === 0) ? "AL" : String(numVal).padStart(2, '0');
                return `${stPo}-${cdStr}`;
            }
        }
    }
    ...
    if (p.STATEFP) {
        ...
        let cdNum = p.CDFP || p.CD || p.CD118FP || p.CD119FP || p.CD117FP || p.CD116FP || p.CD115FP;
    ...
    try {
        ...
        async function initDistrictLayer(vintage) {
			...
			try {
                ...
                // convert lat/long to screen space before calculating county weights
                const firstGeom = geojson.features[0]?.geometry;
                ...
                if (firstPt && Array.isArray(firstPt) && isFinite(firstPt[0]) && firstPt[0] < 0 && firstPt[0] >= -180) {
                    isGeographic = true;
                }
